...
#import "EGHardDecoderController.h"
...
@interface EGHardDecoderController()
{
    NSThread *thread;
    uint8_t* _vdata;
    size_t _vsize;
...
@implementation EGHardDecoderController
...
- (void)stop {
    ...
    if (_thread != nil) {
        if (!_thread.isCancelled) {
            [_thread cancel];
            NSLog(@"uid:%u thread cancel", _uid);
        }
    }
