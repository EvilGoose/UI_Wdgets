...
#import "EGComprehensiveController.h"
#import "FKRecordManager.h"

@interface EGComprehensiveController ()
<FKRecordManagerDelegate>
...
@implementation EGComprehensiveController
        ...
        if ([self.recordManager setupSession])
          {
            ...
            CGRect bounds = self.videoPreviewView.bounds;
                // 设置预览Layer的大小和位置
            previewLayer.frame = CGRectMake(0, 0, SCREEN_WIDTH - 32, SCREEN_HEIGHT * .75);
                // 设置预览Layer的缩放方式
...
- (void)updateButtonStates
{
    ...
    // 实现FKViewController的FKRecordManagerDelegate类别中的方法 (FKRecordManagerDelegate)
    // 拍照或拍摄视频出错时调用该方法
    ...
    CFRunLoopPerformBlock(CFRunLoopGetMain(), kCFRunLoopCommonModes, ^(void)
                          {
                            ...
                            [self.recordButton setTitle:@"拍摄视频" forState:UIControlStateNormal];
                            self.recordButton.enabled = YES;
                          });
}
